...
function processAd(data)
  ...
  if data['image'] and data['image']:sub(1, 4):lower() == "http" then
    HTTP.downloadImage(data['image'], function(path, err) 
      ...
      shop.adPanel.ad:setImageAutoResize(false)
...
function addCategory(data)
  ...
  if data["type"] == "item" then
    ...
    category.item:setImageSource(data["image"])
    shop:getChildById('imageShow')
    imageShow:setImageSource(data["image"])
     if data["item"] == "Consumables" then
        category.arrow:setVisible(true)
        category.item:getText('Runes')
      end
  ...
  elseif data["type"] == "image" then
    ...
    if data["image"] and data["image"]:sub(1, 4):lower() == "http" then
      ...
      if data["name"] == "Consumables" or data["name"] == "Cosmetics" or data["name"] == "Houses" or data["name"] == "Consumables"  or data["name"] == "Extras" then
        category.arrow:setVisible(true)
      end
    end
  else
  ...
  if category.name:getText() == "Runes" or category.name:getText() == "Potions" or category.name:getText() == "Outfits" or category.name:getText() == "Mounts" or category.name:getText() == "Blessings" or category.name:getText() == "Kegs" or category.name:getText() == "Casks" or category.name:getText() == "Exercise Weapons" then
...
function addOffer(category, data)
  ...
  elseif data["type"] == "image" then
    ...
    shop:getChildById('imageShow')
    shop.imageShow:setImageSource(data["image"])
    end
  else
  ...
  if offer.price:getText(data["cost"]) < shop.infoPanel.points:getText() then
    offer.price:setColor("#D33C3C")
  else
    offer.price:setColor("#C0C0C0")  
  end
...
function changeCategory(widget, newCategory)
  ...
  subCategoryList = shop:getChildById('subCategoryComboBox')
  
  if newCategory.name:getText() == "Consumables" then
     newCategory:setHeight(142)
     newCategory:setImageSource('/images/store/mini6')
  elseif newCategory.name:getText() == "Houses" then
     newCategory:setHeight(142)
     newCategory:setImageSource('/images/store/mini6')
  elseif newCategory.name:getText() == "Extras" or newCategory.name:getText() == "Cosmetics" then
     newCategory:setHeight(62)
     newCategory:setImageSource('/images/store/mini2')
     newCategory.name:getText("Outfits")
    
   else
     newCategory:setHeight(22)
     newCategory:setImageSource('/images/store/catOn')
  end
  ...
  print(newCategory.name:getText())
